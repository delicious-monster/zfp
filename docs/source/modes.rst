...
:ref:`C API <api>` section) for setting and querying them.  The members
...
.. c:member:: int zfp_stream.minexp
  ...
  transform coefficient bit depends on the common floating-point exponent, e,
  that scales the integer coefficients.  If the most significant coefficient
  bit has place value 2\ :sup:`e`, then the number of bit planes encoded
  is (one plus) the difference between e and :c:member:zfp_stream.minexp.
  As an analogy, consider representing currency in decimal.  Setting
  :c:member:`zfp_stream.minexp` to -2 ensures that amounts are represented
  to cent accuracy, i.e. in units of 10\ :sup:`-2` = $0.01.  This parameter
  governs the *absolute error* in
...
:c:member:`zfp_stream.minbits`,
which is satisfied at the end of encoding by padding zeros).
...
value, g, the absolute difference \|f - g\| is at most 2\ :sup:`minexp`.
